...
ifeq ($(K100_DDR_SIZE),k100_2G_ddr)
  PRODUCT_COPY_FILES += \
	$(LOCAL_PATH)/k100_2G/init.amlogic.board.rc:root/init.amlogic.board.rc 
else
  PRODUCT_COPY_FILES += \
...
ifeq ($(K100_DDR_SIZE),k100_2G_ddr)
	ifeq ($(TARGET_USE_SECURITY_MODE),true)
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot-2G.bin.aml:u-boot.bin	#bootloader.img in OTA package
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot-2G.bin.aml:u-boot.bin.aml
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot-usb-2G.bin.aml:u-boot-usb.bin.aml
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/ddr_init.bin:DDR_ENC.USB
	else
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot-2G.bin:u-boot.bin
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/ddr_init-2G.bin:ddr_init.bin
	endif
else
	ifeq ($(TARGET_USE_SECURITY_MODE),true)
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot.bin.aml:u-boot.bin	#bootloader.img in OTA package
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot.bin.aml:u-boot.bin.aml
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot-usb.bin.aml:u-boot-usb.bin.aml
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/ddr_init.bin:DDR_ENC.USB
	else
		PRODUCT_COPY_FILES += $(LOCAL_PATH)/u-boot.bin:u-boot.bin
	endif
    PRODUCT_PROPERTY_OVERRIDES += \
...
PRODUCT_COPY_FILES += \
	$(LOCAL_PATH)/../common/res/logo/robot.2048x1536.rle:root/initlogo.rle.bak \
	$(LOCAL_PATH)/../common/res/logo/robot.2048x1536_firstinit.rle:root/firstinitlogo.rle

# inherit from the non-open-source side, if present
$(call inherit-product-if-exists, vendor/amlogic/k100/device-vendor.mk)
