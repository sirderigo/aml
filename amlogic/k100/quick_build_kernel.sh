...
#{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{
PROJECT_NAME=k100
KERNEL_DEVICETREE=meson8_k100_1G
if [ "${K100_DDR_SIZE}" == "k100_1G_ddr" ]; then
	KERNEL_DEVICETREE=meson8_k100_1G
elif [ "${K100_DDR_SIZE}" == "k100_2G_ddr" ]; then
	KERNEL_DEVICETREE=meson8_k100_2G
fi
...
cp $OUT/obj/hardware/wifi/broadcom/drivers/ap6xxx/broadcm_40181/dhd.ko $PRODUCT_OUT/system/lib/
...
cp $(KERNEL_OUT)/../hardware/amlogic/touch/gsl_point_id.ko $(PRODUCT_OUT)/system/lib/
...
cd -
out/host/linux-x86/bin/mkbootimg  --kernel $KERNEL_OUT/arch/arm/boot/uImage  --ramdisk $PRODUCT_OUT/ramdisk.img --second $KERNEL_OUT/arch/arm/boot/dts/amlogic/${KERNEL_DEVICETREE}.dtb --output $PRODUCT_OUT/boot.img
...
cd -
out/host/linux-x86/bin/mkbootimg  --kernel $KERNEL_OUT/arch/arm/boot/uImage  --ramdisk $PRODUCT_OUT/ramdisk-recovery.img --second $KERNEL_OUT/arch/arm/boot/dts/amlogic/${KERNEL_DEVICETREE}.dtb --output $PRODUCT_OUT/recovery.img
